...
import { ChevronLeft, ChevronRight, FolderGit2, Pencil, Plus, Star, Trash2 } from 'lucide-react';
...
import { useAdminProjects, useDeleteProject } from '@/hooks/use-admin-queries';
  ...
  return (
    <div className="space-y-5">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <h1 className="text-xl font-semibold text-zinc-100">Projetos</h1>
          <p className="text-sm text-zinc-500 mt-0.5">
            {meta ? `${meta.total} projeto${meta.total !== 1 ? 's' : ''}` : ' '}
          </p>
        </div>
      ...
      <div className="rounded-lg border border-zinc-800 bg-zinc-900/40 overflow-hidden">
        <div className="grid grid-cols-[1fr_auto_auto_auto_auto] items-center px-4 py-2.5 border-b border-zinc-800 bg-zinc-900/80">
          ...
          <span className="text-xs font-medium text-zinc-500 uppercase tracking-wide w-10 text-center">
            ⭐
          </span>
        ...
        {isLoading ? (
          (['rs1', 'rs2', 'rs3', 'rs4', 'rs5', 'rs6'] as const).map((sk) => (
            <RowSkeleton key={sk} />
          ))
        ) : isError ? (
          <div className="flex flex-col items-center gap-3 py-12 text-center">
            <p className="text-sm text-zinc-500">Falha ao carregar projetos</p>
            <Button
              size="sm"
              variant="ghost"
              onClick={() => refetch()}
          ...
          projects.map((project: Project) => (
            ...
            >
              <div className="min-w-0 pr-4">
                ...
                <p className="text-xs font-mono text-zinc-600 truncate">{project.slug}</p>
              </div>
                  ...
                  <AlertDialogContent className="bg-zinc-900 border-zinc-800">
                    <AlertDialogHeader>
                      ...
                      <AlertDialogDescription className="text-zinc-400">
                        <strong className="text-zinc-300">{project.title}</strong> será removido.
                        Esta ação usa soft delete.
                      </AlertDialogDescription>
                    </AlertDialogHeader>
                    <AlertDialogFooter>
                      ...
                      <AlertDialogAction
                        ...
                        onClick={() => deleteProject.mutate({ id: project.id, slug: project.slug })}
                      >
                        Excluir
                      </AlertDialogAction>
                    </AlertDialogFooter>

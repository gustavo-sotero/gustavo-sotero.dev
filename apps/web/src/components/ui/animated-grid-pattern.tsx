...
export interface AnimatedGridPatternProps extends React.SVGAttributes<SVGElement> {
  width?: number;
  height?: number;
  x?: number;
  y?: number;
...
export function AnimatedGridPattern({
  ...
  const [squares, setSquares] = useState<Array<{ id: string; pos: [number, number] }>>([]);

  const getPos = useCallback((): [number, number] => {
    const cols = Math.ceil((dimensions.width + width) / width);
    const rows = Math.ceil((dimensions.height + height) / height);
    return [Math.floor(Math.random() * (cols + 1)), Math.floor(Math.random() * (rows + 1))];
  }, [dimensions, width, height]);

  const generateSquares = useCallback(
    (count: number) =>
      Array.from({ length: count }, () => ({ id: crypto.randomUUID(), pos: getPos() })),
    [getPos]
  );

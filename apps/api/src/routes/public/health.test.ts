...
describe('health routes', () => {
  ...
  it('GET /ready returns readiness success when db and redis are healthy', async () => {
    ...
    const body = (await response.json()) as {
      success: boolean;
      data: { db: string; redis: string };
    };

    expect(response.status).toBe(200);
    expect(body.success).toBe(true);
    expect(body.data).toEqual({ db: 'ok', redis: 'ok' });
  });

  it('GET /ready returns SERVICE_UNAVAILABLE when any dependency fails', async () => {

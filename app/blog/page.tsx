...
export default function BlogPage() {
  ...
  // Get all unique tags
  const allTags = Array.from(new Set(blogPosts.flatMap((post) => post.translations[language].tags))).sort()
    ...
    <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}>
        ...
        <div className="mb-8">
          ...
          <h1 className="text-3xl font-bold tracking-tight sm:text-4xl md:text-5xl mb-4">{t("blog.title")}</h1>
              ...
              <motion.div key={post.id} variants={item} custom={index}>
                <Card className="h-full flex flex-col hover:shadow-lg transition-shadow">
                  ...
                  <CardContent className="flex-grow">
                    <p className="text-muted-foreground line-clamp-3">{post.translations[language].excerpt}</p>
